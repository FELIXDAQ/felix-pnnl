...
#include <linux/seq_file.h>       // EC, 19-Apr-2016
//#include <asm/system.h>           //e.g. for wmb  // EC, 19-Apr-2016
...
/**************************/
{
...
#elseif
  //proc_create_data("io_rcc", 0, NULL, &fops, NULL);
  io_rcc_file = proc_create("io_rcc", 0644, NULL, &fops);  // EC, 18-Apr-2016 for 3.10 and above (as RHEL7)
#endif
  ...
  //MJ: Rubini does not mention the block below in the 2.6 book but he is in
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,10,00)  // EC, 18-Apr-2016 for 3.10 and above (as RHEL7)
...
/**********************************************************************************************/
{
  ...
  {
    kdebug(("io_rcc_mmap(io_rcc_write_procmem): error logging enabled\n"))
    errorlog = 1;
  }

  if (!strcmp(fb_data->value, "noelog"))
  {
    kdebug(("io_rcc_mmap(io_rcc_write_procmem): error logging disabled\n"))
    errorlog = 0;
  }
  return len;
}
