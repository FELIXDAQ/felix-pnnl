...
#define APPLICATION_NAME    "flx-i2c"
...
/*****************************************/
{
  ...
  {
    printf("ERROR: FLX711 is not yet supported\n");
    exit(-1);
  }
...
/****************************/
{
  flx_i2c_list(FLX_710);
  flx_i2c_list(FLX_709);
}
  ...
  switch(mode)
  {
	...
	try
	{
	  ...
	  printf("Now reading from register 0x%02x of device %s\n", i2caddr, argv[2 + cont]);
          flxCard.i2c_devices_read(argv[2 + cont], i2caddr, &result);
	}
	catch(FlxException ex)
	{
	  std::cout << "ERROR. Exception thrown: " << ex.what() << std:: endl; 
	...
	try
	{
	  ...
	  flxCard.i2c_devices_write(argv[2 + cont], i2caddr, i2cdata);
	}
	catch(FlxException ex)
	{
	  std::cout << "ERROR. Exception thrown: " << ex.what() << std:: endl; 
